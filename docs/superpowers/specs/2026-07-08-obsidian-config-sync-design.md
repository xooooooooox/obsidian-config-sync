...
├── config-sync.json                    # 组定义(用户维护,随 Import 整体覆盖;设置面板可一键创建,带 JSON Schema)
...
### config-sync.json（store 根，只含组定义；`$schema` 指向仓库 `schema/config-sync.schema.json`）
...
**externalSources 不在 config-sync.json 里**。它是每库/每设备私有配置，存于插件自身设置（data.json，不入 store）——否则场景 C 的 Import 用上游 config-sync.json 覆写本库时，会把本库配置的外部源清掉。
...
| D4 | **externalSources 入插件设置** | 外部源定义存插件自身 data.json（每库独立），config-sync.json 只含 groups | Import 覆写 config-sync.json 时不摧毁本库的外部源配置 |
...
2. Import 覆写本库 `<root>/`（config-sync.json、store.lock.json、store/，含删除传播）→ 接着 Apply 落地；
...
四个命令均配 ribbon（Import 仅桌面端显示）；设置面板提供 config-sync.json 一键创建（含 `$schema` 引用与 snippets/hotkeys 起始组，已存在不覆盖）。
