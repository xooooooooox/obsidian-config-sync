...
- **Settings sync** — the item's own file-level sharing rule, the same icon-plus-this-device shape as above (hover the shared glyph for `Default settings sync: All devices` / `Desktop only` / `Mobile only`, and this device's own glyph for `This device: not synced here` when you've opted this one out). A fields-mode item has no whole-file rule to move, so its shared cell shows the italic note `Per-key rules decide — see More` instead of a glyph.
