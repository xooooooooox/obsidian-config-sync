...
describe("itemsIn — hostile about the shapes that pass for the wrong reason", () => {
  it("refuses a beta section: beta is presented, never stored (spec §7b)", () => {
    expect(() => itemsIn({ beta: { "slides-rup": { enabled: true } } } as never)).toThrow(/beta/);
  });

  it("refuses a v2-style compound id — nesting carries the section now", () => {
    expect(() => itemsIn({ community: { "community:dataview": { enabled: true } } })).toThrow(/bare item id/);
    expect(() => itemsIn({ custom: { "community/x": { enabled: true } } })).toThrow(/bare item id/);
  });

  it("accepts a bare id in a stored section, and fills the rest empty", () => {
    const items = itemsIn({ community: { dataview: { enabled: true } } });
    expect(items.community).toEqual({ dataview: { enabled: true } });
