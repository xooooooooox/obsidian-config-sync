...
- **明确、可回退的 Apply** —— 挑选规则组，查看版本不匹配警告，再落地；每个被改动的文件都会被备份，**Revert last apply**（撤销上次应用）可以将其还原。
- **清楚掌握同步状态** —— **Status**(状态) 视图会为每个规则组打上徽标（`✓ in sync`、`↑ changed on this device (likely)`、`↓ store is newer (likely)`、`≠ differs`、`— not captured yet`），同步菜单会显示变更数量，Apply 选择器会预先勾选 store 中已更新的项目。
- **感知远程状态** —— 检查 git 或 vault 远程仓库是否在你的本地 store 之后被捕获过；可按需 Pull/Push（仅桌面端）。
- **移动端友好** —— capture、apply、status 在手机上均可正常工作；store 本身就是普通的 vault 内容，因此任何笔记同步工具都能携带它。
...
2. 运行 **Capture: save this device's settings**（功能区菜单或命令面板）。
3. 在另一台设备上，等你的笔记同步工具(note sync)把数据文件夹送达之后：运行 **Apply: update this device with synced settings**。
...
- **Capture**（捕获） 把 `<数据文件夹>/config-sync.json` 中定义的规则组复制进 `<数据文件夹>/store/`，剥离被 `sanitize` 标记的键，跳过操作系统垃圾文件，并把源插件版本号记录到 `store.lock.json` 中。
- **Apply**（应用） 挑选规则组，对插件版本不匹配的情况发出警告，然后把它们落地到本设备的配置目录（不论其名称是什么）。被脱敏的键会保留本设备的本地值。单槽位备份覆盖每一个被改动的文件；**Revert last apply** 可将其还原。
- **Status**（状态） 按规则组比较实时配置与 store，给出尽力而为的方向提示（文件时间对比最近一次 capture），并支持按需检查远程仓库的新鲜度。

**传输层面** —— store 如何流转：

- **你的笔记同步工具（默认）**：store 本身就是普通的 vault 内容——remotely-save、Obsidian Sync、iCloud 或其他任何工具都能把它带到任何地方，包括移动端，零配置。
- **Pull / Push（桌面端，可选）**：config-sync 自带的传输通道，用于 git 仓库或本机上的另一个 vault。Pull 会用远程内容覆盖本 vault 的 store（可重复执行——冷启动和日常使用是同一条命令）；Push 则把内容发送出去。git 传输方式会克隆到一个临时目录，绝不会触碰你 vault 自身的 git 仓库。

一切功能都挂在一个 **Config Sync** 功能区图标上，点击会打开一个菜单，列出当前可用的操作，并在有待处理项时显示变更数量（例如 `Capture (2 changed here)`）。也可以在 **Settings → General** 中为各个命令单独启用功能区图标，默认关闭。

![Status](docs/assets/status-modal.png)

## 设置指南

- **General** —— PKM 模式（自动检测 IOTO vault）、数据文件夹位置、状态提示开关（菜单变更数量、Apply 选择器徽标）、功能区图标。
...
2. 运行 **Capture: save this device's settings**。
3. 在其他每台设备上，等笔记同步工具把数据文件夹送达后，运行 **Apply: update this device with synced settings**。
...
2. 勾选想同步的内容，执行 Capture，交给 remotely-save 传输；其他设备执行 Apply。
...
2. 运行 **Pull: get settings from a remote**，然后运行 **Apply: update this device with synced settings**。
3. 之后，在源 vault 中运行 **Push: send settings to a remote**，发布更新供其他 vault 拉取。
...
这两个功能在你配置远程之前都处于禁用状态，并且只有在你明确执行 Pull 或 Push 命令时才会运行。
