...
- Added a per-remote passphrase for a remote vault that encrypts with a different one: link it once in that remote's editor (the same keychain-backed picker as the access token), and comparison opens that remote's copies with its own key. Left empty — the default — the two vaults are assumed to share one passphrase and everything behaves exactly as before
- Changed `Can't compare` to say whose key failed, because the two send you to different fixes: your own passphrase missing points at Settings → General, while a remote whose copy doesn't open names the remote and points at its own settings row — and says whether that's because no passphrase was linked for it, or because the linked one doesn't open its copy
- Fixed the direction counts wearing the wrong colours while a remote is selected: a waiting pull is cyan with the cloud glyph and a waiting push is pink, everywhere they appear — the sidebar badges, the filter pills, each row's own arrow and the picker — matching the buttons that run them. On the `This device` view nothing changes
- Changed the view picker's closed control to carry no counts at all — the `All items` row is where the census lives, one screen-line below, and each remote's own push/pull pair rides its row in the picker's dropdown (a remote that can't be counted item by item keeps its whole-store state icon; one still being checked shows a pulsing dot). The lone aggregate pill that floated at the top of the panel is gone — the status bar already carries that number
- Added a pulsing dot where a remote's counts will land while it is still being compared, so an empty badge no longer looks like nothing to do
- Changed the sidebar to lead with a two-row masthead: **Config Sync** with its own state pill on top — the way into the plugin's own pane, its pill shown on the `This device` view only, where it has something to answer for — and the view picker beneath, an icon and a single name (`This device`, or the remote's own name) with the full relation in its hover. The search sits under the masthead, then the sections; the panel header keeps only the refresh
- Changed the four-stop direction control on an item's card (and on each key rule) into the card's own trigger-and-menu: the current stop's icon with the whole story in its hover, and the stops as a menu with words — and a key rule's menu also carries **Remove rule**, which returns that key to following its item
- Changed an item's card to group what belongs together: **This remote** with **Settings sync** directly under it, then the changed files with **Key rules** beneath them — a key rule acts on keys inside those very files. The key document sits behind the same eye Settings uses, its hint underlining `key` the way the clickable keys themselves are; clicking keys works whatever the item's own direction
- Changed the conflict choice to live where the evidence is: `Use theirs` / `Keep mine` sit inside the expanded **Files** block, under the entries and whichever diff you open — one control on the whole panel, replacing both the separate Resolve row and the copy every open diff's toolbar drew. At rest the pair is as quiet as the card's other controls, direction colour on the icons alone; the side you pick takes the colour and the fill. A multi-file item says right beneath that it is written as a whole
- Added an honest answer while a remote hasn't been checked yet this session: the panel says `Checking …` and kicks the check itself, instead of presenting an all-clear that silently flipped once the startup check landed half a minute later
- Fixed the panel flashing on every rules edit: a check sweep's repaints coalesce into one, a settled comparison keeps drawing while its refresh runs quietly underneath, and an open key document repaints only when the file actually changed
- Fixed changing a rule from inside an item's open card dropping that card out of the active filter (or folding it away) mid-gesture — the row now stays where you are looking until you close its card or navigate
- Fixed the remote Pull/Push buttons rendering their text in a colour that fought the background — they now wear their intended solid direction colours with readable text
- Fixed the expanded file list in an item's card starting at an indent of its own — it now lines up with every other row's value column
- Changed the card's per-key area to say `Key rules`, the same words the Settings drawer already uses for the same idea
- Changed the encrypted marker to tell the two shapes of encryption apart: an item stored as one sealed file keeps its `encrypted` chip, while an item whose individual values are encrypted now wears its own `encrypted keys` chip — the first travels whole or not at all, the second still offers its keys for per-key choices
- Added re-encryption in transit: encrypted items now travel to and from a remote that keeps its own passphrase, converted so each vault only ever stores what its own key opens. The first exchange converts each file once; after that an unchanged item moves nothing, so a repeated push stays silent instead of rewriting every encrypted file
- Added honest per-item skips when a key doesn't open something: a wrong or unlinked passphrase skips just the affected items, each with its own line in the report, and never writes anything unreadable into either side's store
