...
- Changed the view picker's closed control to carry just the direction pair, the same two badges its own dropdown shows; the full bucket counts stay on the filter pills, and the view's name no longer gets squeezed into an ellipsis. The lone aggregate pill that floated at the top of the panel is gone — each remote's dropdown row and the status bar already carry that number
...
- Changed the sidebar to lead with the view picker: choose what you're looking at, then filter it. The picker now reads as an icon and a single name — `This device`, or the remote's own name — with the full relation in its hover, and carries the current view's own counts on its right edge. The panel header keeps only the across-all-remotes totals and the refresh, and the duplicate `to capture` chip is gone: the pinned Config Sync row was already the way there
- Changed the four-stop direction control on an item's card (and on each key rule) into a segmented control: every stop visible at once, the current one lit, one click to change — no menu in between
- Changed an item's card to group what belongs together: the direction control first, then the changed files with `Key rules` directly beneath them — a key rule acts on keys inside those very files — and the document now shows open under its hint the way Settings shows it, clipping past a dozen lines behind `Show all N keys`
