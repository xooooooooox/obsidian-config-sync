...
- Changed what happens before re-encryption in transit exists: encrypted items don't travel to or from a remote that keeps its own passphrase. Each one is skipped with its own line in the report instead of writing something unreadable into either side's store
