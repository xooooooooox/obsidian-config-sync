...
describe("planImport / applyImport", () => {
  it("local-only group and its store file survive a pull untouched", async () => {
    ...
    const pending = await planImport(ctx, fakeReader(remote), { skipRefs: [], withheldPull: () => [] });
  ...
  it("planImport writes nothing (read-only)", async () => {
    ...
    await planImport(ctx, fakeReader(remote), { skipRefs: [], withheldPull: () => [] });
  ...
  it("throws when choices.length does not match the number of conflicts", async () => {
    const { ctx } = setup();
    await writeGroups(ctx, [HOTKEYS_GROUP]);
    const pending = await planImport(ctx, fakeReader({}), { skipRefs: [], withheldPull: () => [] });
    expect(pending.plan.conflicts).toEqual([]);
  ...
  it("excludeSelf: a divergent self store copy is neither a conflict nor written by pull", async () => {
    ...
    expect((await planImport(ctx, fakeReader(remote), { skipRefs: [], withheldPull: () => [] })).plan.conflicts.length).toBe(1);

    const pending = await planImport(ctx, fakeReader(remote), { skipRefs: [SELF_ITEM_REF], withheldPull: () => [] });
    expect(pending.plan.conflicts).toEqual([]);
    await applyImport(ctx, pending, []);
...
describe("pushExternal", () => {
  it("writes the whole local store to the remote with deletion propagation and finalizes once", async () => {
    ...
    const results = await pushExternal(ctx, fw.writer, { skipRefs: [], withheldPush: () => [], expectPush: [] });
    expect(results.every((r) => r.status === "ok")).toBe(true);
  ...
  it("refuses to push when the local store has no captured data at all", async () => {
    ...
    await seedGroups(ctx, '{"version":1,"groups":[]}');
    const fw = fakeWriter({});
    await expect(pushExternal(ctx, fw.writer, { skipRefs: [], withheldPush: () => [], expectPush: [] })).rejects.toThrow("capture from this device");
  });
  ...
  it("excludeSelf: push neither writes the local self copy nor mirror-deletes the remote's", async () => {
    ...
    const results = await pushExternal(ctx, fw.writer, { skipRefs: [SELF_ITEM_REF], withheldPush: () => [], expectPush: [] });
  ...
  it("withheld item: the far end keeps its own lock entry, and ours never lands on top of it", async () => {
    ...
    await pushExternal(ctx, fw.writer, { skipRefs: [SELF_ITEM_REF], withheldPush: () => [], expectPush: [] });
    const pushed = parseStoreLock(fw.files["store.lock.json"] ?? "");
...
// spec 2026-08-20-remote-direction-rules-design.md §3.1 / §3.4: a key that does not pull with this
// remote keeps this vault's own value, and the item that results is a hybrid neither side's lock
// entry describes.
describe("pull with per-key withholding", () => {
  ...
  const DEMO_REL = "cs/store/configdir/plugins/demo/data.json";
  const demoDoc = (theme: string, localOnly: string): string => JSON.stringify({ theme, localOnly }, null, 2) + "\n";
  const BARE_LOCK = JSON.stringify({ capturedAt: "t", items: {}, version: 3 });

  it("takes the remote's other keys and leaves a withheld key at this vault's own value", async () => {
    const { io, ctx } = setup();
    io.seed({ "cs/store.lock.json": BARE_LOCK, [DEMO_REL]: demoDoc("mine", "keep-me") });
    await seedGroups(ctx, MANIFEST_DEMO);
    const groups = await readGroups(ctx);
    const reader = fakeReader({ "store.lock.json": BARE_LOCK, "store/configdir/plugins/demo/data.json": demoDoc("theirs", "theirs") });
    const pending = await planImport(ctx, reader, { skipRefs: [], withheldPull: withheldPatternPredicate(RULES, "pull", groups) });
    // Both sides have the file and they differ, so it is a conflict like any other — but what the
    // "theirs" side offers is already the merged content, which is what a pull would actually write.
    expect(pending.plan.conflicts.length).toBe(1);
    await applyImport(ctx, pending, ["remote"]);
    expect(JSON.parse(await io.read(DEMO_REL))).toEqual({ theme: "theirs", localOnly: "keep-me" });
  });

  it("writes nothing when only the withheld key differs", async () => {
    const { io, ctx } = setup();
    io.seed({ "cs/store.lock.json": BARE_LOCK, [DEMO_REL]: demoDoc("same", "mine") });
    await seedGroups(ctx, MANIFEST_DEMO);
    const groups = await readGroups(ctx);
    const reader = fakeReader({ "store.lock.json": BARE_LOCK, "store/configdir/plugins/demo/data.json": demoDoc("same", "theirs") });
    const pending = await planImport(ctx, reader, { skipRefs: [], withheldPull: withheldPatternPredicate(RULES, "pull", groups) });
    expect(pending.plan.auto.writeFiles).toEqual([]);
    expect(pending.plan.conflicts).toEqual([]);
  });
  ...
  it("re-stamps a merged item instead of adopting the remote's entry for it", async () => {
    ...
    const reader = fakeReader({ "store.lock.json": remoteLock, "store/configdir/plugins/demo/data.json": demoDoc("theirs", "theirs") });
    const pending = await planImport(ctx, reader, { skipRefs: [], withheldPull: withheldPatternPredicate(RULES, "pull", groups) });
    await applyImport(ctx, pending, ["remote"]);
    const entry = lockEntry(parseStoreLock(await io.read("cs/store.lock.json"), groups), "community/demo");
...
// spec §3.7's third layer: the panel's judgement can be a refresh cycle old while the push reads the
// far end fresh. What the user picked is ITEMS, not bytes.
describe("push with a stale pick", () => {
  ...
  const entry = (capturedAt: string, hash: string): Record<string, unknown> => ({ ...pluginSource("1.0.0"), capturedAt, hash });
  const lockOf = (capturedAt: string, e: Record<string, unknown>): string =>
    JSON.stringify({ capturedAt, items: { community: { demo: e } }, version: 3 });

  const setupPair = async (remoteEntry: Record<string, unknown>): Promise<{ ctx: CoreContext; fw: ReturnType<typeof fakeWriter> }> => {
    const { io, ctx } = setup();
    io.seed({
      "cs/store.lock.json": lockOf(AT, entry(AT, "sha256:mine")),
      "cs/store/configdir/plugins/demo/data.json": '{"theme":"mine"}',
    });
    await seedGroups(ctx, MANIFEST_DEMO);
    const fw = fakeWriter({
      "store.lock.json": lockOf(LATER, remoteEntry),
  ...
  it("refuses to push a picked item the far end has moved past since the comparison", async () => {
    const { ctx, fw } = await setupPair(entry(LATER, "sha256:theirs"));
    await expect(
  ...
  it("says nothing about an item the caller never claimed to have judged", async () => {
    const { ctx, fw } = await setupPair(entry(LATER, "sha256:theirs"));
    await pushExternal(ctx, fw.writer, { skipRefs: [], withheldPush: () => [], expectPush: [] });
    expect(fw.writeLog).toContain("store/configdir/plugins/demo/data.json");
  });

  it("lets a picked item through when the far end has not moved past it", async () => {
    const { ctx, fw } = await setupPair(entry(AT, "sha256:theirs"));
    await pushExternal(ctx, fw.writer, { skipRefs: [], withheldPush: () => [], expectPush: ["community/demo"] });
    expect(fw.writeLog).toContain("store/configdir/plugins/demo/data.json");
  });
});
