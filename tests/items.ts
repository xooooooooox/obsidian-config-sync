...
// Fixture builder for the v3 nested item store (spec 2026-08-11-v3-one-vocabulary-design.md §2):
...
//
// - a `beta` section. `beta` is a presented classification, never a stored one (spec §7b); a beta
...
export function itemsIn(partial: Partial<Record<StorageSection, Record<string, Item>>>): ItemMap {
  for (const [section, items] of Object.entries(partial)) {
    if (section === "beta") {
      throw new Error('itemsIn: "beta" is not a stored section (spec §7b) — a beta plugin\'s item lives in "community"');
    }
