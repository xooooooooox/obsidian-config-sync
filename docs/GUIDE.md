...
- **Files** — collapsed behind a small count (hover: `N files change`); click it to expand a `+`/`~`/`−` entry per file (added / updated / removed), in both directions — hover any entry for what side it lands on and what happens (`New in the store — starts syncing to your other devices`, `Deleted from this device`, and so on). The row's own leading badge (hover it) says which side the whole list affects, once — into the store while capturing, onto this device while applying. A diffable/viewable entry ends in one small icon; click it to view the change (or the incoming content, if there's nothing local yet to diff against). Encrypted content reads `changed — encrypted, no preview` instead.
...
- **Enabled on** (plugins, while the Core/Community on/off list itself is a synced item) — two icons side by side, each a click-to-open menu with a tooltip on hover (a small up-down mark appears when you hover the row), no wordmark on the row itself: the shared answer's icon (hover it for `Default enabled on: All devices` / `Desktop only` / `Mobile only` / `Each device decides`) and, beside it under a muted **this device** label, this device's own state. Most of the time the second glyph just points back to the default (hover: `This device: follows the default`) — there's nothing to say, because this device does whatever the shared answer says. The moment you tell it to do something else here, that glyph turns purple (`This device: on here` / `off here`) and opens a menu to change it. See [Each device decides, and this device's own exceptions](#each-device-decides-and-this-devices-own-exceptions) below for what that second glyph means and where else it shows up.
...
- **Settings sync** — the item's own file-level sharing rule, the same icon-plus-this-device shape as above (hover the shared glyph for `Default settings sync: All devices` / `Desktop only` / `Mobile only`, and this device's own glyph for `This device: not synced here` when you've opted this one out). An item under per-key rules has no whole-file rule to move, so its shared cell is a dim settings icon (hover: `Per-key rules decide — opens Settings`) that jumps straight to the item's own card.
...
Whatever the shared rule says, **this device can still say something different** — a plugin you always want on here regardless of what the rest of the fleet does, or one you'd rather leave off on just this machine. That's "leave it to me on this device," and it lives in one place, the same place everywhere the plugin's row appears — its own card, the on/off list's own card, the Sync Center row: right beside the shared answer, under a muted **this device** label. Hover it and it reads `This device: follows the default` until you say otherwise, then `This device: on here` or `off here` in purple once you have. It never travels: setting it here doesn't touch your other devices, and a Pull or an Apply from another device can't reset it out from under you.
...
- The path row's own sharing icon and lock disappear — there is no whole-file rule left to show once per-key rules govern the keys individually — replaced by a dim jump icon that scrolls straight down to the Key rules list (its tooltip reads **Per-key rules decide — jump to them**; the **Key rules** heading just below it makes the same point permanently). The row's own **this device** opt-out column (above) stays exactly where it was; it still stops the whole file, per-key rules included.
- A **Key rules** list appears with a row per configured key: its own sharing icon, a lock toggle, and the same **this device** column as the path row on every key that has one (see **This device's own exception** in [Field rules & sensitive settings](#field-rules--sensitive-settings) below for what setting it does, and which keys don't get one). A key that can't be encrypted — its rule is `Each device decides`, or Per-item device rules are on — shows no lock at all. To remove the rule, open the key's sharing menu and pick **Remove rule** at the bottom.
...
- **This device's own exception** — beside a per-key rule's sharing icon, and beside the whole-file row's (above), sits a second, one-device-only column: turn it on and this device stops syncing that key — or, on the whole-file row, that entire file — full stop. Whatever the shared answer is, the value already sitting in the store is left exactly as found: an `All devices` key keeps the shared value, a `Desktop only`/`Mobile only` key keeps the one in its device-class sidecar, and an encrypted key keeps its existing envelope. This device neither publishes its own copy over any of them nor deletes them, and no other device is affected either way. It's the same **this device** column and the same two-choice menu (`Follows the default` / `Not synced here`) as the Sync Center's own `Settings sync` row, and it never travels. A key with **Per-item device rules** on has no such column: its items are governed one at a time by their own rules, and those have no this-device layer for an exception to act on.
...
4. Open the Appearance card's `snippets/` companion folder to give any snippet its own enablement rule: `All devices` / `Desktop only` / `Mobile only` (shared, travels, and is enforced on the other device class) / `Each device decides` (no shared answer — each device keeps whatever it already has). A plugin's **Enabled on** zone works the same way for which devices turn it on (a desktop-only plugin's menu skips the `Mobile only` stop).
